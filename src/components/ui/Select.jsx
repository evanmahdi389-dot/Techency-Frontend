...
const Select = ({ label, value, onChange, options, required, className = '', containerClassName = '', error }) => {
  return (
    <div className={`flex flex-col ${containerClassName}`}>
      ...
      <select
        className={`p-3 bg-[#e0e2e6] border ${error ? 'border-red-500' : 'border-slate-200'} rounded-lg w-full focus:ring-2 focus:ring-[#002546] outline-none text-[#002546] font-bold ${className}`}
        value={value}
        onChange={onChange}
        required={required}
      >
