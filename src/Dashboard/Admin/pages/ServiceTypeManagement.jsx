...
export default function ServiceTypeManagement() {
  const [settings, setSettings] = useState({ serviceTypes: [] });
  ...
  const fetchSettings = async () => {
    try {
      ...
      setSettings(res.data || { serviceTypes: [] });
    } catch (err) { toast.error('Failed to load service types'); }
  ...
  const handleSaveModal = async () => {
    ...
    if (!val) { toast.error('Service Type cannot be empty'); return; }

    const currentItems = settings.serviceTypes || [];
    let newItems;

    if (modalMode === 'add') {
      if (currentItems.includes(val)) return toast.error('Service Type already exists');
      newItems = [...currentItems, val];
    } else if (modalMode === 'edit') {
      if (val !== editingItem && currentItems.includes(val)) return toast.error('Service Type already exists');
      newItems = currentItems.map(i => (i === editingItem ? val : i));
    }

    setSaving(true);
    try {
      const res = await api.get('/settings/dynamic');
      const updatedSettings = { ...(res.data || {}), serviceTypes: newItems };
      await api.put('/settings/dynamic', updatedSettings);
      setSettings(updatedSettings);
      toast.success(modalMode === 'add' ? 'Service Type added!' : 'Service Type updated!');
      setIsModalOpen(false); setItemInput('');
    } catch (err) { toast.error('Failed to save Service Type'); }
  ...
  const handleDelete = async () => {
    ...
    const currentItems = settings.serviceTypes || [];
    const newItems = currentItems.filter(i => i !== itemToDelete);
    ...
    } catch (err) { toast.error('Failed to delete Service Type'); }
    finally { setSaving(false); }
  };

  const filteredItems = (settings.serviceTypes || []).filter(item => item.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  return (
    <div className="flex flex-col h-full font-sans text-[#002546] pb-10">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-8 gap-4">
        <div>
          <h1 className="text-[26px] font-extrabold tracking-tight text-[#002546]">Service Types</h1>
          <p className="text-[#A3AED0] font-bold text-sm mt-1">Manage global service types (e.g. Studio, Outdoor)</p>
        </div>
          ...
          <table className="w-full text-left text-sm whitespace-nowrap border-collapse">
            <thead>
              <tr className=' bg-[#1a3b59]'>
                ...
                <th className="px-6 py-4 font-bold text-xs uppercase tracking-wider text-[#A3AED0] text-center border-b border-[#F4F7FE]">Service Type</th>
            ...
            <tbody>
              {filteredItems.length > 0 ? (
                ...
                <tr>
                  <td colSpan="3" className="px-6 py-10 text-center font-bold text-[#A3AED0]">No service types found.</td>
                </tr>
              )}
            </tbody>
          </table>
          ...
          <div className="bg-white border border-slate-100 rounded-[20px] p-8 w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-[#002546] font-extrabold text-[20px]">
                {modalMode === 'add' ? 'Add Service Type' : 'Edit Service Type'}
              </h3>
            ...
            <div className="mb-6">
              <label className="block text-xs text-[#A3AED0] font-bold mb-2 uppercase tracking-wider">Type Name</label>
              <input
                type="text"
                value={itemInput}
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#002546]/40 backdrop-blur-sm p-4">
          <div className="bg-white border border-slate-100 rounded-[20px] p-8 w-full max-w-sm shadow-2xl text-center">
            ...
            <h3 className="text-[20px] font-extrabold text-[#002546] mb-2">Delete Type?</h3>
