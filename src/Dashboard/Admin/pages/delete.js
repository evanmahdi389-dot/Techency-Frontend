        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredVideos.map(video => {
            ...
            return (
              ...
              >
                {/* Automatically Generates Thumbnail by skipping initial black frames */}
                {/* {video.drive_file_id ? (
                  <video
                    src={`https://docs.google.com/uc?export=download&id=${video.drive_file_id}`}
                    preload="metadata"
                    muted
                    playsInline
                    className="w-full h-full object-cover pointer-events-none"
                    onLoadedData={(e) => {
                      // শুরুতে কালো ফ্রেম এড়াতে ভিডিওর ১.৫ সেকেন্ডের মাথায় গিয়ে থাম্বনেইল পজিশন সেট করা হয়েছে
                ...
                {/* ==================================================== */}
                {/* Fixed Google Drive Thumbnail Bypass */}
                {video.drive_file_id ? (
                  <img
                    src={`https://images1-focus-opensocial.googleusercontent.com/gadgets/proxy?url=${encodeURIComponent(`https://drive.google.com/thumbnail?id=${video.drive_file_id}&sz=w800`)}&container=focus&refresh=2592000`}
                    alt={video.title}
                    className="w-full h-full object-cover"
                    onError={(e) => {
                      // যদি ড্রাইভের নিজস্ব প্রসেসিং এখনো রানিং থাকে, তবে একদম নিখুঁত একটি ডার্ক ব্যাকগ্রাউন্ড দেখাবে
                      e.target.onerror = null;
                      e.target.src = 'https://placehold.co/600x800/111111/666666?text=Processing+Media';
                    }}
                  />
                ) : video.thumbnail_url ? (
                  <img src={video.thumbnail_url} alt={video.title} className="w-full h-full object-cover" />
