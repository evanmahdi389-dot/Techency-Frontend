...
import { FiUpload, FiX, FiVideo, FiSearch, FiEdit, FiEye, FiTrash2 } from 'react-icons/fi';
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-white/5">
              {loading ? (
                ...
                <tr>
                  <td colSpan="5" className="py-10 text-center text-gray-500">No videos found</td>
                </tr>
              ) : (
                filteredVideos.map(video => (
                  <tr key={video._id} className="hover:bg-white/5 transition-colors group">
                      ...
                      <div className="flex items-center gap-4">
                        <div className="w-24 h-14 bg-[#1a1a1a] rounded-lg overflow-hidden flex-shrink-0 relative">
                          {video.thumbnail_url ? (
                            ...
                            <div className="w-full h-full flex items-center justify-center text-gray-600 text-[10px] text-center p-1">
                              {video.title}
                            </div>
                          )}
                        </div>
                    ...
                    <td className="py-4 px-6 text-center text-white text-sm font-medium">
                      {video.category || 'N/A'}
                    </td>
                    <td className="py-4 px-6 text-center text-white text-sm font-medium">
                      {video.subcategory || 'N/A'}
                    </td>
                    <td className="py-4 px-6 text-center">
                      <span className={`text-[10px] px-2.5 py-1 rounded-full border capitalize ${STATUS_COLORS[video.status] || 'text-gray-400 border-gray-500/20'}`}>
                        {video.status}
                      </span>
                    </td>
                    <td className="py-4 px-6 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button className="w-8 h-8 rounded-lg bg-white/5 hover:bg-white/10 text-white flex items-center justify-center transition-colors">
                          <FiEdit className="w-4 h-4" />
                        </button>
                        <button className="w-8 h-8 rounded-lg bg-white/5 hover:bg-white/10 text-gray-400 hover:text-white flex items-center justify-center transition-colors">
                          <FiEye className="w-4 h-4" />
                        </button>
                        <button
                          onClick={() => setDeleteModal(video._id)}
          ...
          <div className="bg-[#151515] border border-white/10 rounded-2xl w-full max-w-2xl shadow-2xl my-auto">
            <div className="flex justify-between items-center p-6 border-b border-white/10">
              ...
              <button
                ...
                className={`cursor-pointer text-gray-400 hover:text-red-500 transition-colors ${uploading ? 'opacity-50 cursor-not-allowed' : ''}`}
                disabled={uploading}
              >
            ...
            <div className="p-6">
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div
                  ...
                  className={`border-2 border-dashed rounded-2xl p-8 text-center transition-all ${uploading ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'
                    } ${dragOver ? 'border-red-500 bg-red-500/5' : file ? 'border-green-500/50 bg-green-500/5' : 'border-white/10 hover:border-white/20 bg-[#111]'
                    }`}
                >
                  <input id="admin-video-input" type="file" accept="video/*" className="hidden" onChange={e => handleFile(e.target.files[0])} disabled={uploading} />
                  {file ? (
                    <div className="flex items-center justify-center gap-3 text-green-400">
                      ...
                      {!uploading && (
                        <button type="button" onClick={(e) => { e.stopPropagation(); setFile(null); }} className="ml-2 text-gray-500 hover:text-red-400">
                    ...
                    <div className="text-gray-500">
                      ...
                      <p className="font-medium text-gray-400">Drag & drop video here</p>
                      <p className="text-sm mt-1">or click to browse — MP4, MOV, AVI (max 500MB)</p>
                    </div>
                  )}
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Organic Honey Product Ad"
                    disabled={uploading}
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="organic, product, ad, 2024"
                    disabled={uploading}
                    className="w-full bg-[#111] border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-red-500 transition-all disabled:opacity-50"
                  />
                </div>
                ...
                <div className="pt-2 flex gap-3">
                  <button
                    ...
                    disabled={uploading}
                    className="flex-1 py-3.5 border border-white/10 text-gray-400 hover:text-white font-medium rounded-xl transition-all disabled:opacity-50"
                  >
                    Cancel
                  </button>
                  <button
                  ...
                  >
                    {uploading ? (
                      ...
                      <><FiUpload className="w-5 h-5" /> Upload to Drive</>
                    )}
                  </button>
                </div>
              </form>
